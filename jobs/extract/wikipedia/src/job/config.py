...
class AppConfig:
    ...
    def _parse_numeric_fields(self):
        ...
        try:
            ...
            raise InvalidJobConfigurationError(field_name="api_page_size", value=self.api_page_size, message="Invalid integer value")
        
        try:
            self.max_pages_per_interval = int(self.max_pages_per_interval)            
        except ValueError:
            raise InvalidJobConfigurationError(field_name="max_pages_per_interval", value=self.max_pages_per_interval, message="Invalid integer value")
        
        try:
            self.api_rate_limit_per_second = int(self.api_rate_limit_per_second)
        except ValueError:
            raise InvalidJobConfigurationError(field_name="api_rate_limit_per_second", value=self.api_rate_limit_per_second, message="Invalid integer value")
        ...
        try:
            ...
            if isinstance(self.interval_end, str):
                self.interval_end = datetime.fromisoformat(self.interval_end.replace("Z", "+00:00"))
        except ValueError:
            raise InvalidJobConfigurationError(field_name="interval_start/interval_end", value=f"start={self.interval_start}, end={self.interval_end}", message="Invalid datetime format, expected ISO 8601")
