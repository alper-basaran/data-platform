...
from job.lib.parquetutils import convert_to_parquet_bytes, build_partitioned_key
...
def run(config: AppConfig):
    ...
    logger.info(f"Persisted {len(all_changes)} events to s3://{config.s3_bucket}/{output_key}.")
